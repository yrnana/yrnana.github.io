...
## `const { reset } = useQueryErrorResetBoundary()`의 `reset` 함수는 invalidate를 trigger 하는 함수가 아니다
...
```ts
const { refetch } = useQuery({ enabled: false, useErrorBoundary: true });
```

이런 경우는 가능한 이러한 쿼리의 `cacheTime`을 짧게 잡거나, 아니면 `useMutation`을 쓰는게 맞는 것 같다.

## useQuery에서 반환하는 refetch를 사용할 때 오류 옵션을 확인하자

`throwOnError`라는 옵션이 있는데 기본값이 `false`이다.
이 값이 없으면 refetch 발생시 오류가 발생하더라도 로직이 catch 되지 않는다 (ㅠㅠ)
